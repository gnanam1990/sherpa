...
const BALANCE_FILLER_WORDS = new Set([
  'balance',
  'balances',
  'what',
  'whats',
...
function isBalanceishWord(word: string): boolean {
  const singular = word.endsWith('s') ? word.slice(0, -1) : word;
  return singular === 'balance' || damerauLevenshteinDistance(singular, 'balance') <= 2;
}

function isLikelyBalanceAsk(raw: string): boolean {
  ...
  if (!words.some(isBalanceishWord)) return false;

  return words.every((word) => BALANCE_FILLER_WORDS.has(word) || isBalanceishWord(word));
}
...
export function parseDeterministic(input: string): ParsedIntent {
  ...
  if (BALANCE_RE.test(raw) || isLikelyBalanceAsk(raw)) {
