...
confirmation, and routes supported Base mainnet actions through audited smart
contracts.
...
| Aave V3 lending actions | Live   | Supply, borrow, repay, and withdraw through the audited router         |
...
- 2 external audit rounds completed
- 0 critical findings
...
See `SECURITY.md` for the public security policy and audit summary.
...
- Bridge functionality is disabled until adapters are audited.
...
Thanks to the external reviewers who helped harden the Stage 2 contracts and to
the Base, Aave, Aerodrome, Farcaster, Mini App, Viem, Wagmi, Foundry, and
Next.js ecosystems that Sherpa builds on.

This release is intentionally honest: what is live is marked live, what is
read-only is marked read-only, and what is blocked on future audit or platform
availability is called out directly.
